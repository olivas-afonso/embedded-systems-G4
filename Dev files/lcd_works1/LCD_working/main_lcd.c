...
int main(void)
{
    ...
    ROM_UARTConfigSetExpClk(UART1_BASE,g_ui32SysClock,9600,(UART_CONFIG_WLEN_8 | UART_CONFIG_STOP_ONE | UART_CONFIG_PAR_NONE));
    ...
    while(1)
    {
        if(UARTCharsAvail(UART1_BASE)){
                  Lcd_Write_String((char*)UARTCharGetNonBlocking(UART1_BASE));
        }
